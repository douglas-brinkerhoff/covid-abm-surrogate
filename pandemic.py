...
axes_to_round = [2,3,5,6,7,13,17,19,20,21,22,23,24,25]
keys_to_round = [keys[i] for i in axes_to_round]
import numpy as np

if __name__ == '__main__':
    mse_arr = []
    parameters =  pm.parameter_creator('params/model_param.txt') 
    samples = pickle.load(open('../../covid-nn/mcmc_samples/sample_1.p','rb'))
    recorder = analysis.recorder(['tests_performed_total', 'positive_tests_total', 'active_cases'], 'parameter_3.hdf5')
    for i in range(10):
        sample = dict(zip(keys,samples[i]))#parameters.randomized_sample()
        for k in keys_to_round:
            sample[k] = int(np.round(sample[k]))
        sample['scenario_name'] = 'trial_'+str(i)
        sample['quarantining'] = 1
        sample['social_distancing'] = 1

        #print(sample)
        pandemic = Disease(sample)# setting empty dict of values
        #print(sample)
